        ...
        window.addEventListener("scroll", (() => {
            ...
            if (sections.length && navLinks.length) {
                sections.forEach((section => {
                    ...
                    if (scrollY >= sectionTop - sectionHeight / 3) currentSection = section.getAttribute("id");
                }));
                navLinks.forEach((link => {
                    link.classList.remove("menu__item_active");
                    if (link.getAttribute("href") === `#${currentSection}`) link.classList.add("menu__item_active");
                }));
            }
        }));
        window["FLS"] = true;
